...
class CartController {
  async create(req, res, next) {
    ...
    // discountedPrice = discountedP * quantity * packagingSize;
  ...
  async getCart(req, res, next) {
    ...
    for (const c of cart) {
      if (c.discount && !validDiscount(c.discount)) {
        c.price = c.price;
        c.discountedPrice = c.discountedPrice;
        delete c.discount;
    ...
    const mappedData = {
      ...
      totalAmount:  _.sumBy(cart, 'discountedPrice') + _.sumBy(cart, 'taxAmount') + shippingCharge,
