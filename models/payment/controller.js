...
class PaymentController {
  ...
  async paymentCapture(req, res, next) {
    ...
    let response;
    let order;
    // let paymentId = 'pay_LCFiBLRn90DnPW'
    try {
    ...
    try {
      response = await razorPay.payments.capture(paymentId, order.totalAmount * 100, "INR");
    } catch (error) {
      next(error)
      if (error.error.description == 'This payment has already been captured') {
        next('Payment already captured');
        return res.send({ statusCode: 200, message: 'Payment already captured' })
      }
      next(error.error.description)
    ...
    const e = new Error('Response status is not paid');
    e.statusCode = 400;
    return next(e);
  }
}
